...
public class Team {
	...
	private Optional<ActivePokemon> activePokemon;
	...
	public void setDefaultActivePokemon() {
		activePokemon = Optional.of(new ActivePokemon(Stream.of(team).filter(p -> p.isAlive()).findFirst()));
	}
	...
	public boolean setActivePokemon(int index) throws IndexOutOfBoundsException {
		...
		if (team[index].equals(activePokemon.get().getPokemon())) {
			return false;
		}
		
		this.activePokemon = Optional.of(new ActivePokemon (team[index]));
		return true;
	}
